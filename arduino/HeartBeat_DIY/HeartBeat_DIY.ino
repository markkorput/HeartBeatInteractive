...
void loop(){
  ...
  for(i=199; i>0; i--){//search for the low and high out of the last 200 samples
  ...
  mapped = data[0];
//  mapped = map(data[0], 0, 1023, 0, 255);
  Serial.write(mapped);//send over the current value to the processing sketch, but scale it to match the screen height
