...
void readData(){
  ...
  if (myPort.available() <= 0) return;  // If data is available,
...
//  val = int(trim(inString));//convert the string into a number we can use
//  println(inString + " / "+ val + " / " + inString.length());
  strokeWeight(2);//beef up our white line
  ...
  line(old_x, old_y, screen_increment, 400-val);
